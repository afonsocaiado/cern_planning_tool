...
def get_data_activities():
    """
    This function reads raw CSV files extracted from the company's databases,
    processes and merges the data, and then generates the final 'activity.csv' file.
    """

    # Read raw CSV files
    activity_list = pd.read_csv('.\original_tables\\activity_list.csv', encoding='unicode_escape')
    activity = pd.read_csv('.\original_tables\\activity.csv', encoding='unicode_escape')
    phase = pd.read_csv('.\original_tables\phase.csv', encoding='unicode_escape')
    hl_schedulable_phase = pd.read_csv('.\original_tables\hl_schedulable_phase.csv', encoding='unicode_escape')
    period = pd.read_csv('.\original_tables\period.csv', encoding='unicode_escape')
    hl_sched_prop_period = pd.read_csv('.\original_tables\hl_sched_prop_period.csv', encoding='unicode_escape')

    # Process ACTIVITY LIST DataFrame
    ...
    activity_list = activity_list.drop(columns=['ACTIVITY_TYPE_FR', 'PRIORITY_FR', 'STATUS', 'RESPONSIBLE_ORG_UNIT', 'RESPONSIBLE_PERSON_ID', 'RESPONSIBLE_FULL_NAME', 'HAS_DATA_QUALITY', 'HAS_NOT_OK_COMMENT_FOLLOW_UP', 'LAST_UPDATE_DATE', 'PRIORITY_EN'])
    ...
    # Form 'activity_df' by merging DataFrames
    ...
    # Drop unnecessary columns (Uncomment this line if needed)
    activity_df = activity_df.drop(columns=['PERIOD', 'PREPARATION_DURATION', 'INSTALLATION_DURATION', 'COMMISSIONING_DURATION']) # Could be used for later predictions

    # Write the final 'activity.csv' file
    activity_df.to_csv('..\\activity.csv', index=False)

def get_data_contributions():
    """
    This function reads raw CSV files extracted from the company's databases,
    processes and merges the data, and then generates the final 'contributions.csv' file.
    """

    # Read raw CSV files
    contribution_config_ref = pd.read_csv('.\original_tables\\contribution_config_ref.csv', encoding='unicode_escape')
    contribution_field = pd.read_csv('.\original_tables\\contribution_field.csv', encoding='unicode_escape')
    contribution_group = pd.read_csv('.\original_tables\\contribution_group.csv', encoding='unicode_escape')
    contribution_group_period = pd.read_csv('.\original_tables\\contribution_group_period.csv', encoding='unicode_escape')
    contribution_type = pd.read_csv('.\original_tables\\contribution_type.csv', encoding='unicode_escape')
    period = pd.read_csv('.\original_tables\period.csv', encoding='unicode_escape')
    phase = pd.read_csv('.\original_tables\phase.csv', encoding='unicode_escape')
    org_unit = pd.read_csv('.\original_tables\org_unit.csv', encoding='unicode_escape')
    org_unit_import = pd.read_csv('.\original_tables\org_unit_import.csv', encoding='unicode_escape')
    
    # Process CONTRIBUTION CONFIG REF DataFrame
    contribution_config_ref = contribution_config_ref.drop(columns=['PLAN_UUID', 'VERSION_UUID']) # based on uniques

    # Process CONTRIBUTION FIELD DataFrame
    contribution_field = contribution_field.drop(columns=['PLAN_UUID', 'VERSION_UUID', 'PRIMARY_FIELD_CODE']) # based on uniques

    # Process CONTRIBUTION GROUP DataFrame
    ...
    contribution_group = contribution_group.drop(columns=['FOR_COORDINATION'])

    # Process CONTRIBUTION TYPE DataFrame
    ...
    # Process ORG UNIT IMPORT DataFrame
    ...
    contribution_group_and_phase = contribution_group_and_phase.drop(columns=['UUID_to_fetch', 'GROUP_UUID', 'SECTION_UUID'])
    ...
    # Write the final 'contributions.csv' file
    contribution_group_and_phase_and_type.to_csv('..\\contributions.csv', index=False)
